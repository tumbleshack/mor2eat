    ...
    FS: FnMut(&N) -> bool,
{
    ...
    let mut ki: usize = 0;
    while routes.last().unwrap().cost <= time_limit {
        if routes.len() <= ki {
        ...
        // println!("Previous = {:?}, with ki = {}", previous, ki);
        ...
        for i in 0..(previous.len() - 1) {
            ...
            for path in routes.iter() {
                // println!("path = {:?}", path);
                if path.nodes.len() >= i && &path.nodes[0..i] == root_path && path.nodes.len() > i + 1 {
